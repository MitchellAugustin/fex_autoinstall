...
def main():
    ...
    if ExecutionCommand == Command.UNBREAK:
        ...
        WgetInstallArgs = copy.deepcopy(ChrootArgs)
        WgetInstallArgs.append("/usr/bin/add-apt-repository")
        WgetInstallArgs.append("-y")
        WgetInstallArgs.append("ppa:canonical-nvidia/nvidia-desktop-edge")
        WgetInstallResult = subprocess.run(WgetInstallArgs)

        AptArgs = copy.deepcopy(ChrootArgs)
        AptArgs.append("/usr/bin/apt")
        AptArgs.append("update")
        AptResult = subprocess.run(AptArgs)

        AptArgs = copy.deepcopy(ChrootArgs)
        AptArgs.append("/usr/bin/apt")
        AptArgs.append("upgrade")
        AptArgs.append("-y")
        AptResult = subprocess.run(AptArgs)

        WgetInstallArgs = copy.deepcopy(ChrootArgs)
        WgetInstallArgs.append("/usr/bin/apt")
        WgetInstallArgs.append("install")
        WgetInstallArgs.append("-y")
        WgetInstallArgs.append("ubuntu-drivers-common")
        WgetInstallResult = subprocess.run(WgetInstallArgs)

        WgetArgs = copy.deepcopy(ChrootArgs)
        WgetArgs.append("/usr/bin/ubuntu-drivers")
        WgetArgs.append("install")
        WgetResult = subprocess.run(WgetArgs)
        
        ChmodArgs = copy.deepcopy(ChrootArgs)
        my_env = os.environ.copy()
        my_env["DEBIAN_FRONTEND"] = "noninteractive"
        ChmodArgs.append("/usr/bin/apt")
        ChmodArgs.append("install")
        ChmodArgs.append("-y")
        ChmodArgs.extend(["nvidia-driver-580-open", "pulseaudio", "libgles1", "libgles2", "libglx-mesa0", "libgl1-mesa-dri", "mesa-utils", "mesa-utils-extra", "libglx-mesa0", "libgl1"])
        ChmodResult = subprocess.run(ChmodArgs, env=my_env)
